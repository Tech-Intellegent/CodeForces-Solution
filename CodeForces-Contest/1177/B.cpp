...
int32_t main()
{
    ...
    vll p;
    ll nw=get(ans-1);
    while(ans) p.eb(ans%10),ans/=10;
    rev(p);
    for(i=0;i<sz(p);i++){
        nw++;
        if(nw==k) return cout<<p[i]<<nl,0;
    }
    assert(0);
    return 0;
}
