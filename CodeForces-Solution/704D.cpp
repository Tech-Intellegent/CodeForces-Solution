...
struct LR_Flow {
    ...
    void add_edge(int u, int v, int l, int r, int id = -1) {
  	...
  	pair<bool, int> feasible(int _s = -1, int _t = -1, int L = -1, int R = -1) {
   		...
   		if (ans < target) return {0, -1}; //not feasible
   		return {1, ans};
   }
   int max_flow(int _s, int _t) { //-1 means flow is not feasible
   		int mx = 1e5 + 9;
   		return feasible(_s, _t, 0, mx).second;
   }
