...
bitset<mxn>bs[26],allset,ans;
int main()
{
    fast;
    int i,j,k,n,m,q,l,r,ind,type,yo;
    char ch;
    string s,sub;
    cin>>s>>q;
    n=s.size();
    for(i=0;i<n;i++) allset[i]=1,bs[s[i]-'a'][i]=1;
    while(q--){
        cin>>type;
        if(type==1){
            cin>>ind>>ch;
            ...
            for(i=0;i<m;i++) ans&=(bs[sub[i]-'a']>>i);
            yo=(ans>>(l-1)).count()-(ans>>(r-m+1)).count();
            cout<<yo<<nl;
        }
    }
    return 0;
